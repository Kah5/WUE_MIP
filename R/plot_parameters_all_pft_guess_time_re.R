...
for(i in 1:length(spec)){
  ...
  ggsave(paste0("outputs/gwbi_model/LPJ_GUESS_time_re/acfplot_", spec[i],".png"), plot = acf)
...
for(i in 1:length(spec)){
  ...
  acf <- ggplot(ests.summary[!ests.summary$params %in% c("sigmaY", "beta5.1.", "beta5.2."),], aes(params, mean ))+
 ...
 ggsave( paste0("outputs/gwbi_model/LPJ_GUESS_time_re/parameter_ests_", spec[i],".png"), plot = acf)
...
INTERCEPTS <- ggplot(all.param.summary[all.param.summary$variable %in% "alpha",], aes(species, mean))+geom_point()+
  geom_point()+geom_errorbar(aes(min = Ci.low, max = Ci.high), width = 0.1)+geom_hline(aes(yintercept = 0), color = "grey", linetype = "dashed")+
  ylab("Species intercept \n Alpha Estimate")+xlab("Species")+theme_bw(base_size = 15)+theme(panel.grid = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))
...
MAPS <- ggplot(all.param.summary[all.param.summary$variable2 %in% "beta1",], aes(species, mean, color = timeclass))+geom_point()+
  ...
  ylab("Precipitation Sensitivity \n (Beta1) Estimate")+xlab("Species")+theme_bw(base_size = 15)+theme(panel.grid = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))


JUNTMAX <- ggplot(all.param.summary[all.param.summary$variable2 %in% "beta2",], aes(species, mean, color = timeclass))+geom_point()+
  geom_point()+geom_errorbar(aes(min = Ci.low, max = Ci.high, color = timeclass), width = 0.1)+geom_hline(aes(yintercept = 0), color = "grey", linetype = "dashed")+
  ylab("Jun Tmax Sensitivity \n (Beta2) Estimate")+xlab("Species")+theme_bw(base_size = 15)+theme(panel.grid = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))

PREVRWI_1 <- ggplot(all.param.summary[all.param.summary$variable2 %in% "beta3",], aes(species, mean, color = timeclass))+geom_point()+
  geom_point()+geom_errorbar(aes(min = Ci.low, max = Ci.high, color = timeclass), width = 0.1)+geom_hline(aes(yintercept = 0), color = "grey", linetype = "dashed")+
  ylab("PrevRWI_1 \n (Beta3) Estimate")+xlab("Species")+theme_bw(base_size = 15)+theme(panel.grid = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))

PREVRWI_2 <- ggplot(all.param.summary[all.param.summary$variable2 %in% "beta4",], aes(species, mean, color = timeclass))+geom_point()+
  geom_point()+geom_errorbar(aes(min = Ci.low, max = Ci.high, color = timeclass), width = 0.1)+geom_hline(aes(yintercept = 0), color = "grey", linetype = "dashed")+
  ylab("PrevRWI_1 \n (Beta4) Estimate")+xlab("Species")+theme_bw(base_size = 15)+theme(panel.grid = element_blank(), axis.text.x = element_text(angle = 45, hjust = 1))

legend.mp <- get_legend(PREVRWI_2)

png(height = 10, width = 5, units = "in", res = 300, "outputs/gwbi_model/LPJ_GUESS_time_re/All_spec_parameters_by_time.png")
plot_grid(
  plot_grid(INTERCEPTS+theme(legend.position = "none"), MAPS+theme(legend.position = "none"), JUNTMAX+theme(legend.position = "none"), PREVRWI_1+theme(legend.position = "none"), PREVRWI_2+theme(legend.position = "none"), align = "hv",ncol = 1, labels = "AUTO"), 
  legend.mp, ncol = 2, rel_widths = c(1, 0.25))
dev.off()
...
test.data.names <- list.files(path = "outputs/gwbi_model/train_test_data/", pattern = "*_nimble.rds")
index.first <- substring(test.data.names, first = 1, last = 4)
test.dfs <- test.data.names[index.first %in% "test"]
test.dfs.full <- paste0("/Users/kah/Documents/WUE_MIP/WUE_MIP/outputs/gwbi_model/train_test_data/", test.dfs)
...
pred.get.rsq.time <- function(x){
  ...
  for(i in 1:length(meanMAP.sim$Temp.jun.scaled)){
    ...
    int.1[,i] <- int.mcmc.dat[,paste0("alpha")]+
      ...
      int.mcmc.dat[,paste0("beta3.", meanMAP.sim[i,"time_num"], ".")]*meanMAP.sim[i,]$GWBI_1  + 
      int.mcmc.dat[,paste0("beta4.", meanMAP.sim[i,"time_num"], ".")]*meanMAP.sim[i,]$GWBI_2 #+
  ...
  meanMAP.sim$idval <- 1:length(meanMAP.sim$lon)
  
  # rows are the mcmc values
  colnames(int.1) <- 1:length(meanMAP.sim$lon)
  test.m <- melt(int.1)
  ...
  # 
  # 
  indiv.summary <- full.pred %>% group_by( PFT, Year) %>% dplyr::summarise(mean.pred = mean(RWI.pred, na.rm =TRUE),
                                                                                ci.low.pred=quantile(RWI.pred, 0.025, na.rm =TRUE),
                                                                                ci.high.pred=quantile(RWI.pred, 0.975, na.rm =TRUE),
                                                                                mean.obs = mean(GWBI, na.rm =TRUE), 
                                                                                ci.low.obs=quantile(GWBI, 0.025, na.rm =TRUE),
                                                                                ci.high.obs=quantile(GWBI, 0.975, na.rm =TRUE))
  
  saveRDS(full.pred, paste0("outputs/gwbi_model/LPJ_GUESS_time_re/test_", unique(indiv.summary$PFT),"_full_pred.rds"))
  summary.lm <- summary(lm(RWI.pred ~ GWBI, data = full.pred))
  
  site.model.fit <- data.frame(spec = unique(indiv.summary$PFT),
...
saveRDS(rsq.site.df,"outputs/gwbi_model/LPJ_GUESS_time_re/rsq.df.rds")
