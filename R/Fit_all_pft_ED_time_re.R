...
# get the unique spec.codes:
gwbi.clim.nona <-readRDS("/Users/kah/Documents/WUE_MIP/WUE_MIP/Data/ED2_gwbi_pft_clim.rds")
gwbi.full <- gwbi.clim.nona[!is.na(gwbi.clim.nona$GWBI_1) & !is.na(gwbi.clim.nona$GWBI_2)  ,]
spec.list  <- as.character( unique(gwbi.full$PFT) )
...
clim.reg.re.period <- nimbleCode({ 
 ...
 # mu_beta5 ~ dunif(-2, 2)
    ...
    for(i in 1:N) {
      predictedY[i] <- alpha + beta1[period[i]]*Precip.scaled[i] + beta2[period[i]]*Temp.jja.scaled[i]+ beta3[period[i]]*RWI_1[i] + beta4[period[i]]*RWI_2[i]
...
params <- c("alpha",  "beta1", "beta2", "beta3", "beta4", "sigmaY")


run.nimble.spec <- function(spec, train.test.dir,niter, burnin, thin, nchains){
  if(file.exists(paste0("outputs/gwbi_model/ED2_time_re/samps_", spec,niter,"iter_", burnin,"burnin_",thin,"thin" ,".rds"))){
    cat(paste("model for ....", spec, "exists already"))
  }else{cat(paste("Running model for....", spec))
      # readin testing data:
    train.data <-  readRDS(paste0(train.test.dir, "/train_ED2_recent",spec,"_nimble.rds"))
    train.data$GWBI_log <- log(train.data$GWBI)
    ...
    train.data$time_num <- ifelse(train.data$Year <= 1945, 1,2)
      ...
      SPECIES.data <- list(
        ...
        Y = train.data$GWBI_log, Precip.scaled = train.data$Precip.scaled, 
        Temp.jja.scaled = train.data$Temp.jun.scaled, RWI_1 = train.data$GWBI_1, 
        RWI_2 = train.data$GWBI_2, N =length(train.data$GWBI), period = as.numeric(train.data$time_num) )
      
      
      SPECIES.constants <- list( N=length(train.data$GWBI),
      ...
      # save outputs:
      saveRDS(SPECIES.samples, paste0("outputs/gwbi_model/ED2_time_re/samps_", spec,niter,"iter_", burnin,"burnin_",thin,"thin" ,".rds"))
        }
      }
...
run.nimble.spec(spec = "temp.decid.late", 
                 train.test.dir = "outputs/gwbi_model/train_test_data/",
                 niter = 2500, burnin= 200, thin = 2, nchains = 1)
 
# run.nimble.spec(spec = "BIBS.gwbi", 
#                 train.test.dir = "outputs/gwbi_model/train_test_data/",
...
run.nimble.spec(spec = spec.list[i], 
                train.test.dir = "outputs/gwbi_model/train_test_data",
                niter = 28000, burnin= 200, thin = 20, nchains = 1)
