...
hist(GUESS$IWUE)
# remove the outliers:
GUESS.rm <- GUESS[GUESS$IWUE <= 50, ]
ggplot(GUESS.rm, aes(Tair.C, IWUE))+geom_point()

GUESS.rm <- GUESS.rm[!is.na(GUESS.rm$PFT), ]

ggplot(GUESS.rm, aes(IWUE, Rel.Dens))+geom_point(size = 0.5)
GUESS.IWUE.GWBI.PFT <- ggplot(GUESS.rm, aes(IWUE, GWBI, color = PFT))+geom_point(size = 0.5)+facet_wrap(~PFT)

png(height = 6, width = 7, units = "in", res = 300, "outputs/preliminaryplots/GUESS_GWBI_IWUE_by_pft.png")
GUESS.IWUE.GWBI.PFT
dev.off()
...
ggplot(GUESS.rm.site[GUESS.rm.site$Site %in% "1",], aes(Year, IWUE, color = Mean_tair_max_6))+geom_point()+geom_line()+stat_smooth()
...
png(height = 4, width = 10, units = "in", res = 300, "outputs/preliminaryplots/GUESS_delta_wue_precip_tmax_850_1850_compared1950_2011.png")
