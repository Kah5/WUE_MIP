...
# i created a derivative of christys function that only takes one variable, but 
extract.ED.nona <- function(model, model.dir, var, xmin=-100, xmax=-60, ymin=35, ymax=50, yrmin=850, yrmax=2010){
  library(ncdf4); library(abind); library(car); library(reshape2)
  ...
  # PFT labels for ED:
    ...
    if(i < files.use[length(files.use)]){ # if we're going to need another file after this, take everything
      ...
      colnames(datain) <- c('latrow', 'lonrow', var)
    ...
    if(dim(full.out[[var]])[3]%%12==0){ 
      lab.time <- round(yrmin + (1:dim(full.out[[var]])[3] - 1)/12, 3)
    } else {
      lab.time <- yrmin + (1:dim(full.out[[var]])[3] - 1)
    }
    
    # Write in the dimnames
    if(length(dim(full.out[[var]]))==4){
      if(dim(full.out[[var]])[4]==length(pft.lab)){
        dimnames(full.out[[var]]) <- list(lat=lat[ind.lat], lon=lon[ind.lon], time=lab.time, pft=pft.lab)
      } else {
        dimnames(full.out[[var]]) <- list(lat=lat[ind.lat], lon=lon[ind.lon], time=lab.time, soil.layer=soils)
      }
      
    } else {
      dimnames(full.out[[var]]) <- list(lat=lat[ind.lat], lon=lon[ind.lon], time=lab.time)
    }
