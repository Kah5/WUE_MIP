...
dir.clm.cn <- file.path(model.dir, "PBL.CLM45CN")
#files.clm.bgc <- dir(dir.clm.bgc)
files.clm.cn <- dir(dir.clm.cn)

dir.lpj.g <- file.path(model.dir, "PBL_LPJ-GUESS")
...
HeteroResp[[1]] <- data.frame(cbind(clm.cn[["HeteroRest"]], ed[["HeteroResp"]][,s], 
                                    lpj.g[["HeteroResp"]][,s],
                                    lpj.w[["HeteroResp"]][,s], c(jules.s[["HeteroResp"]][,s]*exp(-1.3), rep(NA, 12))))  
AutoResp[[1]] <- data.frame(cbind(clm.cn[["AutoResp"]][,s], ed[["AutoResp"]][,s], lpj.g[["AutoResp"]][,s], lpj.w[["AutoResp"]][,s]), c(jules.s[["AutoResp"]][,s], rep(NA, 12)))  
GPP[[1]] <- data.frame(cbind(clm.cn[["GPP"]][,s], ed[["GPP"]][,s], lpj.g[["GPP"]][,s], lpj.w[["GPP"]][,s]), c(jules.s[["GPP"]][,s], rep(NA, 12)))  
SoilMoist[[1]] <- data.frame(cbind(clm.cn[["SoilMoist"]][,s], ed[["SoilMoist"]][,s], lpj.g[["SoilMoist"]][,s], lpj.w[["SoilMoist"]][,s], c(jules.s[["SoilMoist"]][,s], rep(NA, 12))))  
Qs[[1]] <- data.frame(cbind(clm.cn[["Qs"]][,s], ed[["Qs"]][,s], lpj.g[["Qs"]][,s], lpj.w[["Qs"]][,s], c(jules.s[["Qs"]][,s], rep(NA, 12))))  

names(HeteroResp[[1]]) <- names(AutoResp[[1]]) <- names(GPP[[1]]) <- names(SoilMoist[[1]]) <- names(Qs[[1]]) <- c("clm45", "ed2", "lpj.guess", "lpj.wsl") #, "jules.stat")

# # -------------------------------------
# # Missing JULES_STATIC  
 NEE[[1]] <- data.frame(cbind(clm.cn[["NEE"]][,s], ed[["NEE"]][,s], lpj.g[["NEE"]][,s], lpj.w[["NEE"]][,s]))  
 Evap[[1]] <- data.frame(cbind(clm.cn[["Evap"]][,s], ed[["Evap"]][,s], lpj.g[["Evap"]][,s], lpj.w[["Evap"]][,s]))  
 Transp[[1]] <- data.frame(cbind(clm.cn[["Tranp"]][,s], ed[["Transp"]][,s], lpj.g[["Transp"]][,s], lpj.w[["Tranp"]][,s]))  

names(NEE[[1]]) <- names(Evap[[1]]) <- names(Transp[[1]]) <- c("clm45", "ed2", "lpj.guess", "lpj.wsl")
