...
for(i in 1:length(sites)){
 ...
 TRANSP[[i]]    <- data.frame(t(ncvar_get(transp, sites[i])))
  ...
  names(GPP[[i]]) <- names(AGB[[i]]) <- names(LAI[[i]]) <- names(NPP[[i]]) <- names(NEE[[i]]) <- names(RA[[i]]) <- names(RH[[i]]) <- names(SOILCARB[[i]]) <- names(SOILMOIST[[i]]) <- names(EVAP[[i]])<- names(TRANSP[[i]]) <- names(TAIR[[i]]) <- names(PRECIPF[[i]]) <- names(SWDOWN[[i]]) <- names(LWDOWN[[i]]) <- names(WIND[[i]]) <- names(PSURF[[i]]) <- names(QAIR[[i]]) <- model.names
  
  row.names(GPP[[i]]) <- row.names(AGB[[i]]) <- row.names(LAI[[i]]) <- row.names(NPP[[i]]) <- row.names(NEE[[i]]) <- row.names(RA[[i]]) <- row.names(RH[[i]]) <- row.names(SOILCARB[[i]]) <- row.names(SOILMOIST[[i]]) <- row.names(EVAP[[i]]) <- row.names(TRANSP[[i]]) <- row.names(TAIR[[i]]) <- row.names(PRECIPF[[i]]) <- row.names(SWDOWN[[i]]) <- row.names(LWDOWN[[i]]) <- row.names(WIND[[i]]) <- row.names(PSURF[[i]]) <- row.names(QAIR[[i]]) <- years
  
}
names(GPP) <- names(AGB) <- names(LAI) <- names(NPP) <- names(NEE) <- names(RA) <- names(RH) <- names(SOILCARB) <- names(SOILMOIST) <- names(EVAP) <- names(TAIR) <- names(PRECIPF) <- names(SWDOWN) <- names(LWDOWN) <- names(WIND) <- names(PSURF) <- names(QAIR) <- names(TRANSP) <- sites
