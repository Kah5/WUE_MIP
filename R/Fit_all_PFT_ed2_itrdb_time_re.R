...
rwl.itrdb.clim.nona <- readRDS("Data/ITRDB/full.clim.prism.ED.PFTS.rds")
...
run.nimble.spec <- function(spec, train.test.dir,niter, burnin, thin, nchains){
  ...
  if(file.exists(paste0("outputs/ITRDB_models/ITRDB_ED_PFT_time_re/samps_", spec,niter,"iter_", burnin,"burnin_",thin,"thin" ,".rds"))){
  ...
  cat(paste("Running model for....", spec))
      # readin testing data:
      train.data <-  readRDS(paste0(train.test.dir, "/ED_PFT_train_",spec,"_nimble.rds"))
      ...
      # save outputs:
      saveRDS(SPECIES.samples, paste0("outputs/ITRDB_models/ITRDB_ED_PFT_time_re/samps_", spec,niter,"iter_", burnin,"burnin_",thin,"thin" ,".rds"))
  }
}
