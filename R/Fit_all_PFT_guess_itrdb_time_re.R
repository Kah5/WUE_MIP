...
rwl.itrdb.clim.nona <- readRDS("Data/ITRDB/full.clim.prism.GUESS.PFTS.rds")
...
clim.reg.re.period <- nimbleCode({ 
  ...
  mu_beta5 ~ dunif(-2, 2)
    ...
    for(i in 1:N) {
      predictedY[i] <- alpha + beta1[period[i]]*Precip.scaled[i] + beta2[period[i]]*Temp.jja.scaled[i]+ beta3[period[i]]*RWI_1[i] + beta4[period[i]]*RWI_2[i] + beta5[period[i]]*Age[i]
...
params <- c("alpha",  "beta1", "beta2", "beta3", "beta4", "beta5", "sigmaY")


run.nimble.spec <- function(spec, train.test.dir,niter, burnin, thin, nchains){
  
  if(file.exists(paste0("outputs/ITRDB_models/ITRDB_GUESS_PFT_time_re/samps_", spec,niter,"iter_", burnin,"burnin_",thin,"thin" ,".rds"))){
    cat(paste("model for ....", spec, "exists already"))
  }else{
  cat(paste("Running model for....", spec))
      # readin testing data:
      train.data <-  readRDS(paste0(train.test.dir, "/GUESS_PFT_train_",spec,"_nimble.rds"))
      ...
      # save outputs:
      saveRDS(SPECIES.samples, paste0("outputs/ITRDB_models/ITRDB_GUESS_PFT_time_re/samps_", spec,niter,"iter_", burnin,"burnin_",thin,"thin" ,".rds"))
  }
}
...
# test on a species with small # of data:
 ...
 run.nimble.spec(spec = "Temperate broadleaved summergreen", 
                 train.test.dir = "outputs/ITRDB_models/train_test_data/",
                 niter = 250, burnin= 1, thin = 1, nchains = 1)
# 
...
#FAGR.samps <- readRDS("outputs/ITRDB_models/ITRDB_GUESS_PFT_time_re/samps_Boreal needleleaved evergreen250iter_1burnin_1thin.rds")
 #coda.samples <- as.mcmc(FAGR.samps)
 #summary(coda.samples)
# traceplot(coda.samples)
# acfplot(coda.samples)
